...
class FlatCorrectCP(PixCorrectImStep):
    description = "Apply a flat field correction to an image"
    step_name = config_section

    @classmethod
    def _doit(cls, image, flat_im, rel_gain_for_flat):
        ...
        :Parameters:
            ...
            - `rel_gain_for_flat`: dictionary containing relative gain correction per amplifier
        ...
        try:
            ...
            logger.info('Warning: Assuming that keywords are not present and proceeding with flat')

        # If dictionary has been populated with relative gains then apply
        #  to flat prior to apply flat to the image
        for amp in rel_gain_for_flat:
            logger.info('Scaling flat amplifier %s by %.3f' % (amp, rel_gain_for_flat[amp]))
            sec = section2slice(image['DATASEC' + amp])
            flat_im.data[sec] *= rel_gain_for_flat[amp]
...
#       an empty dictionary is passed here.
    ...
    @classmethod
    @do_once(1, 'DESFLAT')
    def __call__(cls, image, flat_im, rel_gain_for_flat):
        """Apply a flat field correction to an image

        :Parameters:
            - `image`: the DESImage to flatten
            - `flat_im`:  the flat correction image to apply
            - `flat_gaincorrect`:  flag to apply relative gains for backward compatibility in CP

        Applies the correction "in place"
        """
        ret_code = cls._doit(image, flat_im, rel_gain_for_flat)
...
flat_correct_cp = FlatCorrectCP()

# internal functions & classes

if __name__ == '__main__':
    flat_correct_cp.main()
