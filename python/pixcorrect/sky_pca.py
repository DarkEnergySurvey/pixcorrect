...
def process(m, npc):
    ...
    for _ in range(n_clip_iterations):
        work, _ = clip(data, model, n_clip_sigma)
        us, s, v = pca(work, npc)
        model = np.dot(us[:, :npc], v[:, :npc].T)

    # And a final pass with tighter clip
    work, _ = clip(data, model, n_clip_sigma_final)
    us, s, v = pca(work, npc)
...
class SkyPCA(PixCorrectDriver):
    ...
    def __call__(cls, in_filenames, out_filename, npc, reject_rms):
        ...
        for i in range(nexp):
            ...
            rms[i] = mini.rms
        use = rms <= reject_rms
        logger.info('Retained {:d} out of {:d} exposures'.format(np.count_nonzero(use), nexp))
